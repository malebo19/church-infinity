...
if(!isset($_POST["user_id"],$_POST["content"],$_POST["time"]))
{
...
//check group name
$sql = "INSERT INTO `post`(`time`, `content`, `document`,`user_id`) VALUES ('$time','$content','$filePath','$user_id')";
$result = mysqli_query($conn, $sql);
if ($result==false) {
    returnJson(["status"=>false,"message"=>"fail to create a post".mysqli_error($conn)]);
    exit();
}
returnJson(["status"=>true,"message"=>"post created"]);
