...
$sql = "SELECT * FROM `group-members` WHERE `member_id`='$member_id' AND `group_id`='$group_id'";
$result = mysqli_query($conn, $sql);
if (mysqli_num_rows($result)>0) {
    returnJson(["status"=>false,"message"=>"Already a member of the group"]);
    exit();
}

// add the member
$sql = "INSERT INTO `group-members`(`group_id`, `member_id`) VALUES ('$group_id','$member_id')";
if (!mysqli_query($conn, $sql)) {
    returnJson(["status"=>false,"message"=>"fail to create a group".mysqli_error($conn)]);
    exit();
} 
returnJson(["status"=>true,"message"=>"member was added"]);
