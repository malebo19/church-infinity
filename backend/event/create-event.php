<?php 
if(!isset($_POST["user_id"],$_POST["content"],$_POST["title"],$_POST["time"],$_FILES["attachment"]))
{
...
//check group name
$sql = "INSERT INTO `event`(`time`, `content`, `document`,`title`) VALUES ('$time','$content','$filePath','$title')";
$result = mysqli_query($conn, $sql);
if ($result==false) {
    returnJson(["status"=>false,"message"=>"fail to create a event".mysqli_error($conn)]);
    exit();
}
returnJson(["status"=>true,"message"=>"event created"]);
